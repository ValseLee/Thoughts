...
export function portfolioMediaSources(markdown) {
  if (typeof markdown !== "string") throw new Error("portfolio Markdown must be a string");
  ...
  function visit(node) {
    const src = node.type === "image"
      ...
      : node.type === "imageReference"
        ...
        : undefined;
    if (typeof src === "string" && src.startsWith("/portfolio/")) {
      const start = node.position?.start.offset;
      const end = node.position?.end.offset;
      const directMarker = Number.isInteger(start) && Number.isInteger(end)
        ? /^!\[[^\]\n]*\]\((\/portfolio\/[^\s)]+)\)$/.exec(markdown.slice(start, end))
        : null;
      if (node.type !== "image" || directMarker?.[1] !== src) {
        throw new Error("descriptionMarkdown media must use ![alt](/portfolio/file)");
      }
      sources.push(normalizePortfolioSrc(src, "descriptionMarkdown media"));
    }
...
export function normalizePortfolioProject(rawProject) {
  ...
  const media = rawProject.media.map((item, index) => {
  ...
  const mediaSources = new Set(media.map(({ src }) => src));
  for (const src of portfolioMediaSources(rawProject.descriptionMarkdown)) {
    if (!mediaSources.has(src)) {
      throw new Error(`descriptionMarkdown references missing media ${src}`);
    }
  }
